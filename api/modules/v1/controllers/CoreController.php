...
{
    public  $modelClass = 'api\modules\models\member\Member'; 
    private $http_id;
    public  $post;
    public  $get;
    public  $_member;  //用户信息
    ...
    public function beforeAction($action)
    {
       ...
       if(!isset($this->request['auth_key'])) $this->error('auth_key is null.');
        ...
        $this->_member = Member::loginByAuthkey($this->request['auth_key']);        

        if(empty($this->_member)) $this->error('用户不存在','200','404');

        $this->_member = $this->_member->toarray();

        $this->_memberId = $this->_member['member_id']; 
        return true;
    }
    ...
    public function model($model,$params,$scenario='')
    {
        ...
        $classDir  = 'api\\modules\\models'.'\\'.$model;   
        $m = new $classDir();      
        if(!$m){
