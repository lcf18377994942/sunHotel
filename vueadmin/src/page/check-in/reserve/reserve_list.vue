			...
			<el-table
				...
				<el-table-column
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-button type="text" icon="el-icon-document" @click="handleEidt(scope.row.reserve_id)">编辑</el-button>
...
export default{
	...
	methods:{
		getData() {
			...
			this.ifload = true;
			this.$post_('check/reserve/reserve-list',params,(res) => {
				console.log(res);
				if(res.code == '0'){
                    this.list = res.data;
		...
		delData() {
			...
			this.$post_('check/reserve/reserve_del',param,(res) => {
				console.log(res);
				if(res.code ==='0'){
        ...
        addData() {
            this.ifload = true;
            let param = {reserve_id:this.curId};
            this.$post_('check/reserve/check_in_add',param,(res) => {
                if(res.code ==='0'){
                    this.$message.success(res.msg);
                    this.list.splice(this.curIndex,1);
                    this.ifload = false;
		...
		exportExecl() {
			...
			this.$post_('check/reserve/export',params,(res) => {
				console.log(res);
				if(res.code ==='0'){
