...
use apiadmin\modules\models\room\Reserve;
...
class ReserveController extends CoreController
{
    ...
    */
    public function actionReserveList()
    {
        ...
        $params = array(
            'field'	=> ['reserve_id','member_name','room_name','type_name','re.deposit','re.mark','in_time','out_time'],
    ...
    */
    public function actionReserve_del()
    {
        ...
        $reserveInfo = Reserve::getReserveIdById($reserveId);
        $res = Reserve::reserveDel($reserveId);
        if($res){
            //修改用户和房间状态
            MemberModel::setStateId($reserveInfo['member_id'],RoomState::getRoomStateId());
            Room::setStateId($reserveInfo['room_id'],RoomState::getRoomStateId());
            $this->out('退订成功');
        }
        $this->error('退订失败');
    }
    ...
    */
    ...
    {
        if(!$reserveId = $this->request('reserve_id')) $this->error('参数错误');
        $params = Reserve::getReserveIdById($reserveId);
        $checkIn = $this->model('room\CheckIn',$params,'Reg');
        $checkIn->charge = $params['deposit'];
        $checkIn->deposit = Room::getRoomById($params['room_id'])['deposit'];
    ...
    */
    ...
    {
        $params = $this->request;
        $reserve = $this->model('room\reserve',$params,'Edit',$params['reserve_id']);
        $reserve->deposit = bcadd(bcmul($params['price'],$params['discount'],2),Room::getRoomById($reserve['room_id'])['deposit'],2);
        $msg = '';
        if ($params['room_id'] != $params['old_room_id']) {
            $msg .= '【原房间名称：'.$params['room_name'].'】';
            Room::setStateId($params['room_id'],RoomState::getRoomStateId('已预订'));
    ...
    {
        $params = $this->request;
        $reserve = $this->model('room\reserve',$params,'Reg');
        $reserve->deposit = bcadd(bcmul($params['price'],$params['discount'],2),Room::getRoomById($reserve['room_id'])['deposit'],2);
        if(!$reserve->save(false)) $this->error('添加失败');
        //修改用户和房间状态
        MemberModel::setStateId($params['member_id'],RoomState::getRoomStateId('已预订'));
        Room::setStateId($params['room_id'],RoomState::getRoomStateId('已预订'));
        //返回数据
        $this->out('添加成功');
    }
